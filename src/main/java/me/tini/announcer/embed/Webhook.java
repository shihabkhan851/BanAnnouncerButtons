...
public class Webhook {
    ...
    public void setUserFromEmbedData() {
        for (int i = 0; i < embeds.length; i++) {
            Embed.EmbedData data = embeds[i];

            if (data.hasAuthor()) {
                Embed.Author author = data.getAuthor();
                data.removeAuthor();
    ...
    private void sendTo0(String webhookUrl) throws IOException {
        ...
        if (conn.getResponseCode() > 299) {
            throw new IOException("Request failed with response: " + conn.getResponseCode() + " " + conn.getResponseMessage());
        }
